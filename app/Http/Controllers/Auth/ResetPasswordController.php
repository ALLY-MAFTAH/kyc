    ...
    */
    ...
    public function validatePasswordRequest(HttpRequest $request)
    {
        $user =User::where('email', '=', $request->email)
            ->first();
        if (!$user) {
            return redirect()->back()->with('error','User does not exist')->withErrors(['email' => trans('User does not exist')]);
        }
        ...
        $response = $messageingService->sendMessage($user->mobile, $msg);

        // dd($response);
        if ($response['status'] == 'Sent') {
            return back()->with('success', 'A reset link has been sent to your mobile number via sms.');
        } else {
    ...
    public function resetPassword(HttpRequest $request)
    {
        ...
        if (!$user) return redirect()->back()->with('error' , 'Email address not found');
        if ($user->email!=$request->email)
         return redirect()->back()->with('error' , 'This is not correct email you requested for password reset');
        ...
        DB::table('password_resets')->where('email', $user->email)
            ->delete();
       return Redirect::route('login')->with('success','You have successful reset your password');
    }
    /**
